...
contract Pair is IPair {
    ...
    function swap(
        ...
        require(amount0In > 0 || amount1In > 0, "IIA"); // Pair: INSUFFICIENT_INPUT_AMOUNT
        {
            ...
            (address _token0, address _token1) = (token0, token1);
            if (amount0In > 0)
                _update0(IPairFactory(factory).getFeeAmount(stable, amount0In)); // accrue fees for token0 and move them out of pool
            if (amount1In > 0)
                _update1(IPairFactory(factory).getFeeAmount(stable, amount1In)); // accrue fees for token1 and move them out of pool
